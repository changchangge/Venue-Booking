...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        for (var i = 0; i < res.data.length; i++) {
          ...
          param[content] = jsonText[i].Value.building_name;
          that.setData(param);

          var content2 = 'buildings[' + i + '].building_id';
          param[content2] = jsonText[i].Value.id;
          that.setData(param);
  ...
  toNextPage: function (e) {
    ...
    } else if (obj.people_min == '' || obj.people_min == '' || JSON.parse(obj.people_min) > JSON.parse(this.data.people_max)) {
      wx.showToast({
        title: '请选择正确人数',
        icon: 'loading',
        duration: 500,
